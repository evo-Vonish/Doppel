...
- M2 流层追加（SPEC-M2M3 §1.3）：-p 127.0.0.1:0:8080（neko 只发布到宿主 loopback
  的随机空闲端口）、-p 127.0.0.1:59000:59000/udp（WebRTC 单 UDP mux）、
  -e NEKO_PASSWORD=<创建时生成的随机口令>；
...
# WebRTC 媒体单 UDP mux（与镜像 ENV NEKO_UDPMUX=59000 对应）。
# M2 单替身验收使用固定 loopback 端口；多替身端口调度属 D1。
NEKO_UDP_MUX_PORT = 59000
...
def build_run_command(persona: Persona, image_tag: str,
    ...
    if neko_password is not None:
        ...
        # WebRTC 走本机 loopback 的单 UDP mux，口令经环境变量注入容器
        #（persona-bake 第 7.5 步读取，缺失会拒绝启动）。
        args += [
            "-p", f"127.0.0.1:0:{NEKO_CONTAINER_PORT}",
            "-p", f"127.0.0.1:{NEKO_UDP_MUX_PORT}:{NEKO_UDP_MUX_PORT}/udp",
